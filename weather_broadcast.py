...
class WeatherBroadcast:
    ...
    def broadcast(self):
        ...
        get_temperature = re.search(_TEMP, info).group().replace('+', ': +')
        get_feeledtemperature = re.search(_FEEL_LIKE, info).group().replace('+', ' +')
        ...
        try:
            ...
            get_expected = ""
        return "\n".join([get_town, get_temperature, get_feeledtemperature, get_weather, get_expected])

    def change_town(self, Newtown):
        self.town = Newtown

if __name__ == "__main__":
    req = requests.get(_URL + "Moscow")
    info = BeautifulSoup(req.text, "html.parser").get_text()